...
ws = wb.create_sheet('판매셀러')
wa = wb.create_sheet('상품등록셀러')

month = 9

weekSql = f'''
        select week from sell where month = {month} group by week;
        '''
...
productListSql = f'''
        select distinct(`provider_name`) FROM `product` where month = {month} or
        week in ({weekList[0]},{weekList[1]},{weekList[2]},{weekList[3]},{weekList[4]})
        group by `provider_name`
...
for productProviderList in productProviderLists:
    ...
    productSql = f'''
            ...
            (select count(`product_number`) from product where week={weekList[0]} and provider_name = '{product_provider_name}'),
            (select count(`product_number`) from product where week={weekList[1]} and provider_name = '{product_provider_name}'),
            ...
            from product where provider_name = '{product_provider_name}' and month = {month};
        '''
